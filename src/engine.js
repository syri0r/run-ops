 ...
 * Netrun UI (JS) — v2
 * -------------------------------------------------
 * Verdrahtet Engine + Tree mit DOM. Erstellt bei Bedarf Minimal-UI.
 * Enthält Toolbar (NET-Actions), Generator, Programme, Inspector.
...
(function(){
  ...
  // -------- UI scaffold (falls kein Markup vorhanden) --------
  function ensureBaseUI(){
    if($('#treeSvg')) return; // vorhandenes Markup nutzen
    const app = $('#app') || document.body;
    app.insertAdjacentHTML('beforeend', `
      <div id="nr-wrap" style="display:grid;grid-template-rows:auto auto 1fr;gap:8px">
    ...
    $$('#actionBar [data-act]').forEach(btn=>{
      bind(btn,'click',()=>{
        ...
        switch(act){
          ...
          case 'BACKDOOR':{
            if(!spend()) return; // DV/Skill-Wurf könnte hier für aktuellen Knoten erfolgen
            break;
          }
          case 'CONTROL':{ if(!spend()) return; break; }
          case 'ZAP':{ if(!spend()) return; break; }
          case 'SLIDE':{ if(!spend()) return; break; }
          case 'JACKIN':{ /* Markierung/Status */ break; }
        }
